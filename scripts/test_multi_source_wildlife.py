#!/usr/bin/env python3
"""
Test multi-source wildlife data aggregation with GMU and trail mapping.
Combines iNaturalist, eBird, and Observation.org data.
"""
...
from scrapers.ebird_scraper import EBirdScraper
...
def test_multi_source():
 """Test all wildlife data sources together."""
 console.print("\n[bold cyan]Multi-Source Wildlife Data Test[/bold cyan]")
 console.print("Combining iNaturalist, eBird, and Observation.org")
 console.print("=" * 70)
 ...
 console.print("\n[yellow]Fetching wildlife observations from all sources...[/yellow]")
 ...
 source_table.add_column("Percentage", style="green")
 ...
 console.print("\n[bold]Combined Wildlife Sightings (Most Recent):[/bold]")

 table = Table(title="Multi-Source Wildlife Observations", show_lines=True)
 ...
 # GMU hotspot analysis
 console.print("\n[bold cyan]GMU Hotspot Analysis:[/bold cyan]")
 ...
 # Trail proximity analysis
 console.print("\n[bold cyan]Trail Proximity Analysis:[/bold cyan]")
 distances = [s['closest_trail']['distance_miles'] for s in all_sightings]
 avg_distance = np.mean(distances)
 within_1_mile = sum(1 for d in distances if d <= 1.0)
 within_5_miles = sum(1 for d in distances if d <= 5.0)
 ...
 console.print(f"• Within 5 miles of trail: {within_5_miles} ({within_5_miles/len(all_sightings)*100:.0f}%)")

 # Save combined data
 console.print("\n[yellow]Saving combined data...[/yellow]")
 output_file = "data/sightings/multi_source_wildlife.json"

 # Prepare data for JSON (limit to 50 for file size)
 save_data = []
 for s in all_sightings[:50]:
 save_item = s.copy()
 # Flatten nested data for easier JSON serialization
 ...
 console.print(f" Saved combined data to: {output_file}")

 # Final summary
 console.print("\n[bold green]Multi-Source Summary:[/bold green]")
 console.print(f"• Total observations: {len(all_sightings)}")
 console.print(f"• Unique species: {len(species_counter)}")
 console.print(f"• GMUs with activity: {len(gmu_counter)}")
 console.print(f"• Date range: {all_sightings[-1]['observed_date']} to {all_sightings[0]['observed_date']}")
