#!/usr/bin/env python3
"""
Final script to add coordinates to all sightings without them.
            ...
            if 0 <= idx < len(sightings_batch):
                ...
                if coords and isinstance(coords, list) and len(coords) == 2:
                    ...
                    if isinstance(lat, (int, float)) and isinstance(lng, (int, float)):
                        if 36 <= lat <= 42 and -109.5 <= lng <= -102:  # Colorado bounds
...
def generate_final_sql(sightings_with_coords: List[Dict]) -> str:
    """Generate SQL update statements."""
    sql_lines = [
        "-- Final coordinate updates for wildlife sightings",
        f"-- Generated on {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
        f"-- Total updates: {len(sightings_with_coords)}",
        "",
        "BEGIN;",
        ""
    ]
    ...
    sql_lines.extend([
        ...
        "COMMIT;",
        "",
        f"-- Run this query to verify updates:",
        f"-- SELECT COUNT(*) FROM sightings WHERE location IS NOT NULL;"
    ])
...
def main():
    ...
    logger.info("Starting final coordinate update for all sightings...")
    ...
    while offset < total_count:
        response = supabase.table('sightings') \
            ...
            .range(offset, min(offset + fetch_size - 1, total_count - 1)) \
            .execute()
        ...
        logger.info(f"Fetched {len(valid_sightings)} valid sightings ({len(all_sightings)}/{total_count})")
    ...
    logger.info(f"Processing {len(all_sightings)} sightings in batches of {batch_size}...")
    ...
    for i in range(0, len(all_sightings), batch_size):
        ...
        if batch_num % 10 == 1:  # Progress every 10 batches
            elapsed = time.time() - start_time
            rate = i / elapsed if elapsed > 0 else 0
            eta = (len(all_sightings) - i) / rate if rate > 0 else 0
            logger.info(f"Progress: Batch {batch_num}/{total_batches} - {i}/{len(all_sightings)} sightings - ETA: {eta/60:.1f} min")
    ...
    logger.info(f"\n{'='*60}")
    logger.info(f"COORDINATE UPDATE COMPLETE")
    logger.info(f"{'='*60}")
    logger.info(f"Total time: {elapsed_total/60:.1f} minutes")
    logger.info(f"Sightings processed: {len(all_sightings)}")
    logger.info(f"Coordinates added: {len(all_results)}")
    ...
    if all_results:
        ...
        json_file = f'final_coordinates_{timestamp}.json'
        with open(json_file, 'w') as f:
            json.dump({
                'processed': len(all_sightings),
                'successful': len(all_results),
                'timestamp': timestamp,
                'sightings': all_results
            }, f, indent=2)
        
        sql_file = f'final_update_coordinates_{timestamp}.sql'
        with open(sql_file, 'w') as f:
            f.write(generate_final_sql(all_results))
