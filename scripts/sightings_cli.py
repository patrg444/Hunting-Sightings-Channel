    ...
    if date:
        ...
        for s in filtered:
            if s.get('sighting_date'):
                ...
                # Compare dates
                if sighting_date.date() == target_date:
...
def display_sightings(sightings: List[Dict[str, Any]]):
    ...
    for sighting in sightings:
        # Handle both string and datetime for display
        sighting_date = sighting['sighting_date']
        if isinstance(sighting_date, str):
            date_str = sighting_date
        else:
...
def main(date, units, species, lookback, no_scrape):
    ...
    # Use the full GMU data file instead of the sample
